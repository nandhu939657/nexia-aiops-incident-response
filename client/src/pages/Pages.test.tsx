...
describe("multi-page workspaces", () => {
  ...
  it("renders the dedicated incident detail contract", () => { render(<IncidentDetail />); expect(screen.getByText("Incident detail")).toBeInTheDocument(); expect(screen.getByText("Approval and action")).toBeInTheDocument(); expect(screen.getByText("Retrieved runbook")).toBeInTheDocument(); expect(screen.getByText(/step-by-step guide for investigating/)).toBeInTheDocument(); });
  it("renders the runbook reader, activity audit trail, and settings policy panels", () => { render(<Runbooks />); expect(screen.getByText("Step-by-step instructions")).toBeInTheDocument(); cleanup(); render(<Activity />); expect(screen.getByText("Audit trail")).toBeInTheDocument(); cleanup(); render(<Settings />); expect(screen.getByText("Human approval required")).toBeInTheDocument(); });
});
