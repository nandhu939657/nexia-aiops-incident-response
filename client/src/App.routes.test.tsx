...
vi.mock("@/lib/trpc", () => ({ trpc: { useUtils: () => ({ service: { status: { invalidate: vi.fn() } }, incidents: { list: { invalidate: vi.fn() }, get: { invalidate: vi.fn() } }, auth: { me: { invalidate: vi.fn(), setData: vi.fn() } } }), auth: { me: { useQuery: () => ({ data: { id: 1, openId: "test-user", email: "test@example.com", name: "Test User" }, isLoading: false }) } }, service: { status: { useQuery: () => ({ data: { service: "payment-service", status: "healthy" } }) }, simulateFailure: { useMutation: () => ({ mutateAsync: vi.fn(), isPending: false }) }, restore: { useMutation: () => ({ mutateAsync: vi.fn(), isPending: false }) } }, incidents: { list: { useQuery: () => ({ data: [workflowIncident] }) }, get: { useQuery: () => ({ data: workflowIncident, isLoading: false }) }, create: { useMutation: () => ({ mutateAsync: vi.fn(), isPending: false }) }, triggerVoiceAlert: { useMutation: () => ({ mutateAsync: vi.fn(async () => ({ incident: workflowIncident, dispatch: { endpoint: "/api/v1/calls/dispatch" } })), isPending: false }) }, approveRemediation: { useMutation: () => ({ mutateAsync: vi.fn(async () => { workflowIncident = { ...workflowIncident, status: "Resolved", notification: "INCIDENT RESOLVED · payment-service", postMortemMarkdown: "# INC-001\n\n## Follow-up items\n- Review logs" }; return workflowIncident; }), isPending: false }) } }, runbooks: { list: { useQuery: () => ({ data: [runbook] }) } }, jobs: { list: { useQuery: () => ({ data: [{ id: "job-apify-leads", name: "Lead enrichment actor", provider: "apify", externalId: "run_demo_leads_001", status: "failed", lastHeartbeatAt: "2026-08-20T10:00:00.000Z", lastSuccessAt: "2026-08-19T10:00:00.000Z", durationMs: 52300, retryCount: 2, errorMessage: "Actor exceeded memory limit", suggestedAction: "retry", approvalRequired: true, metadata: { actor: "company~lead-enrichment" } }] }) }, get: { useQuery: () => ({ data: undefined }) }, simulateFailure: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, remediate: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, refresh: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, providers: { useQuery: () => ({ data: { internal: { configured: true }, firecrawl: { configured: false }, apify: { configured: false } } }) }, monitorStatus: { useQuery: () => ({ data: { totalRuns: 0, consecutiveUnhealthy: 0, lastResult: "waiting" } }) }, runPaymentMonitor: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } }, monitoring: { testPresets: { useQuery: () => ({ data: [] }) }, checkUrls: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, list: { useQuery: () => ({ data: [] }) }, create: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, pause: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, resume: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, remove: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) }, checkSaved: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } } } }));
